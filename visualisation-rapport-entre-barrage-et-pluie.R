...
barrages$Annee <- year(barrages$Date)

# Agréger les données de pluviométrie par année
pluviometrie_annee <- pluviometrie %>%
  group_by(Annee) %>%
...
barrages_par_annee <- barrages %>%
  group_by(Annee) %>%
  summarise(Total_apports_mm = sum(apports)*1000 )#il faut convertir en mm
...
head(donnees_annee)
# Créer le graphique qui presente l'influence du pluie sur le stock
ggplot(donnees_annee, aes(x = Annee)) +
  geom_line(aes(y = Total_precipitations, color = "Précipitations"), size = 1.5) +  # Courbe représentant les précipitations
  geom_line(aes(y = Total_apports_mm, color = "augmentation du Stock des barrages"), size = 1.5, linetype = "dashed") +  # Courbe représentant le stock des barrages
