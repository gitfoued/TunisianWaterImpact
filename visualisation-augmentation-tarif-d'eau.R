...
Sonned$`2023 (en dt)` <- as.numeric(gsub(",", ".", Sonned$`2023 (en dt)`))
Sonned$`2024 (en dt)` <- as.numeric(gsub(",", ".", Sonned$`2024 (en dt)`))

# Convertir `tranche-metre-cube-par-trimestre` en facteur avec l'ordre approprié
Sonned$`tranche-metre-cube-par-trimestre` <- factor(Sonned$`tranche-metre-cube-par-trimestre`, 
                                                    levels = c("0-20", "20-40", "40-70", "70-100", "100-150", "150 et plus", "usage touristique"))

# Créer le graphique avec les valeurs pivotées
tarif_water_plot <- ggplot(Sonned, aes(x = `tranche-metre-cube-par-trimestre`)) +
